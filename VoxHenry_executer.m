...
disp('------------------------------------------------------------------')
disp('SuperVoxHenry: Inductance Extraction for Voxelized Superconductors')
disp('                                                                  ')
disp('   by Mingyu Wang, Cheng Qian and Abdulkadir C. Yucel (NTU)       ')
disp('                                                                  ')
disp('                 Empowered by modules/ideas from                  ')
disp('           Enrico Di Lorenzo (FastFieldSolvers S.R.L.)            ')
disp('------------------------------------------------------------------')
disp(['Attention: The default method is LDLT. If users require AGMG, please download it via http://agmg.eu/ and   '])
disp(['           put it this path (replace the folder AGMG_3.3.5_aca). the flag to convert LDLT to AGMG is line  '])
disp(['           17 of "lse_sparse_precon_prepare.m" in the "src_lse_formation folder"                           '])
% -------------------------------------------------------------------------
...
% fileinname = inputfilelist(1).name;
...
fl_filling_or_retrieval=0; % 1 for filling circulant tensors
fl_Tucker=0;
tol_Tuck=1e-4;
% -------------------------------------------------------------------------
...
if fl_filling_or_retrieval==0
    ...
    elseif size_up_lim>300 && size_up_lim <=400
        ...
        load('fN_prestored_data_1600_400_100.mat'); % according to the structure size to choose prestored Toeplitz
    end
    tload=toc(loadtime);
    disp([' Total time for load prestored Toeplitz ::: ' ,num2str(tend)]);
    [fN_all2,st_sparse_precon2] = retrieval_circulant1(dx,1,L,M,N,fl_no_fft,fN_prestored_data,fl_Tucker,tol_Tuck);
else
%     [fN_all2,st_sparse_precon2] = lse_generate_circulant_tensor(dx,1,L,M,N,fl_no_fft);
    [fN_all2,st_sparse_precon2] = lse_generate_circulant_tensor(dx,1,L,M,N,fl_no_fft,tol_Tucker,fl_Tucker);
end
...
for freq_no=1:num_freq
    ...
    for ii=1:7
        if fl_Tucker==1
            fN_all=fN_all2; 
            bala=1j*omega*mu;
%             fN_all{ii}{1}= (1j*omega*mu)*fN_all{ii}{1};
        else
            fN_all{ii} = (1j*omega*mu)*fN_all2{ii};
        end
    end
    ...
    tinisim = tic;
%     save('agmg_ldlt.mat','dx','z_real','z_imag','idxS','st_sparse_precon','nodeid_4_grnd','nodeid_4_injectcurr','Ae')
%     return
    ...
    for port_no=1:num_ports
        ...
        fACPU   = @(J)lse_matvect_mult1(J, fN_all, Ae, z_real, z_imag, dx, idxS5, nodeid_4_grnd, nodeid_4_injectcurr,fl_Tucker,bala);
