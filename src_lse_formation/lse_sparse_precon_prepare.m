...
if(exist ('OCTAVE_VERSION', 'builtin') > 0)
    ...
    slct_decomp_sch='ldlt_decomp'; %'ldlt_decomp','lu_decomp','ldlt_decomp','chol_decomp'  %no_decomp means AGMG
end
%slct_decomp_sch='chol_decomp';
...
fl_profile = 1; % cpu and memory profiling
...
%             [LL,p,QQ] = chol(Sch_comp,'lower'); % option 3: LL'=S'AS
...
%              PP1 = colperm(Sch_comp);  
%             %PP1 = amd(Sch_comp);
%             %PP1 = symrcm(Sch_comp);
%             SSS=Sch_comp(PP1,PP1);
%             %[LL,p,QQ] = chol(SSS,'lower'); % option 3: LL'=S'AS
%             [LL,p,QQ] = ldlchol (SSS);
%             infomem1 = whos('LL');infomem2 = whos('p');infomem3 = whos('QQ');
%             memestimated = (infomem1.bytes+infomem2.bytes+infomem3.bytes)/(1024*1024);
%             disp(['Memory for LDLT. fact. matrices (MB)::' , num2str(memestimated)]);
...
%save('Schur_big_struc.mat','Sch_comp','-v7.3'); return;
...
%             Sch_sparse = Sch_comp;
            ...
            Sch_sparse = (Sch_comp);
%             Sch_sparse_real=Sch_sparse;
            Sch_sparse=spfun(@(x)complex(x,1e-8),Sch_sparse);
%             MM=size(Ae,1);NN=size(Ae,2);
%             save('schur.mat','Sch_sparse_real','Sch_sparse','MM','NN')
             agmg((Sch_sparse),[],1,[],[],[],[],1);
