...
%             spparms('spumoni',2)        
            ...
            %[JOut_full_out(num_curr+1:num_curr+num_node),flag,relres,iter,resvec]=agmg((Sch_sparse),(JOut_full_in(num_curr+1:num_curr+num_node) - ( Ae * A_inv * JOut_full_in(1:num_curr) )), ...
             %      50,1e-8,100);
             %       disp(['# iterations of agmg::' , num2str(iter)]);
	    [JOut_full_out(num_curr+1:num_curr+num_node),flag,relres,iter,resvec]=agmg((Sch_sparse),(JOut_full_in(num_curr+1:num_curr+num_node) - ( Ae * A_inv * JOut_full_in(1:num_curr) )), ...
                  1,1e-8,100,[],[],202);
            JOut_full_out(1:num_curr) = (JOut_full_in(1:num_curr) - ((-Ae')*JOut_full_out(num_curr+1:num_curr+num_node))) .* diag(A_inv);
