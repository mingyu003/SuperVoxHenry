function [idx,epsilon_r,sigma_e,lambdaL,grid_intcon,idx_direc] = intcon_constparams(r,Res,Cnt,Dims,Orients,e_r,s_e,l_L,fl_plot_intcons)
        ...
        for nn=1:N
            for kk=1:num_intcon
                ...
                if ( r(ll,mm,nn,1) > x_bnd(kk,1)-tola && r(ll,mm,nn,1) < x_bnd(kk,2)+tola &&...
                    ...
                    idx_direc(counter,1)=ll+L*(mm-1)+L*M*(nn-1);
                    idx_direc(counter,2)=1;
                    idx_direc(counter,3)=kk;
                    elseif Orients(kk)=='y'
                    boolean_tens(ll,mm,nn)=2;
                    idx_direc(counter,1)=ll+L*(mm-1)+L*M*(nn-1);
                    idx_direc(counter,2)=2;
                    idx_direc(counter,3)=kk;
                    elseif Orients(kk)=='z'
                    boolean_tens(ll,mm,nn)=3;
                    idx_direc(counter,1)=ll+L*(mm-1)+L*M*(nn-1);
                    idx_direc(counter,2)=3;
                    idx_direc(counter,3)=kk;
                    end
...
idx_direc(counter+1:end,:)=[];
idx = find(boolean_tens); % get indices of elements
% idx_direc=cell(3,1);
% idx_direc{1}=find(boolean_tens==1);
% idx_direc{2}=find(boolean_tens==2);
% idx_direc{3}=find(boolean_tens==3);
% -------------------------------------------------------------------------
% Assign output
% -------------------------------------------------------------------------
